    ...
    for i in range(len(tab) - 1):
        ...
        for j in range(i + 1, len(tab)):
            if tab[j] < min_elem:
                current = j
                min_elem = tab[j]
        tab[i], tab[current] = tab[current], tab[i]
    ...
    for i in range(len(tab) - 1):
        ...
        for j in range(i + 1, len(tab)):
            if tab[j] < min_elem:
                current = j
                min_elem = tab[j]
        tab.insert(i, tab.pop(current))
        
def main():
    test = int(input())
    if test == 1:
        lst = [(5,'A'), (5,'B'), (7,'C'), (2,'D'), (5,'E'), (1,'F'), (7,'G'), (5,'H'), (1,'I'), (2,'J')]
        tab = [Element(key, value) for key, value in lst]
        tab_1 = tab.copy()
        tab_2 = tab.copy()
        heap = Heap(tab)
        heap.print_tab()
    ...
    if test == 2:
        ...
        swap(tab_1)
        t_stop = time.perf_counter()
        print("Czas obliczeń selection sort (swap):", "{:.7f}".format(t_stop - t_start))

        t_start = time.perf_counter()
        shift(tab_2)
        t_stop = time.perf_counter()
        print("Czas obliczeń selection sort (shift):", "{:.7f}".format(t_stop - t_start))
